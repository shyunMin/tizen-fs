...
## 변경할 아키텍처 및 수정 파일 파악
1. **신규 UI 컴포넌트 추가 (`lib/settings/ethernet_page.dart` [NEW])**
   - 유선 네트워크 정보를 입력받을 수 있는 `EthernetPage` 위젯 추가.
   - 내부적으로 `TextField` 또는 `AutoSuggestionField`와 유사한 입력 폼을 사용하여 IP, Gateway, Subnet Mask, DNS 정보를 입력하는 UI 구성.
2. **설정 메뉴 트리에 반영 (`lib/models/settings_menus.dart` [MODIFY])**
